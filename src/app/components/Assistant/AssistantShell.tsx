...
export default function AssistantShell({
  ...
  return (
    <div className="pointer-events-auto flex h-[min(38rem,78vh)] w-[min(26rem,calc(100vw-1.5rem))] flex-col overflow-hidden rounded-[28px] border border-white/70 bg-[#F4F7F5] shadow-[0_28px_90px_rgba(15,23,42,.28)] animate-[assistantPanelIn_.28s_ease] [font-family:var(--font-noto-deva),var(--font-geist-sans),sans-serif]">
      <div className="relative overflow-hidden bg-gradient-to-br from-[#0B1B16] via-[#102820] to-[#163528] px-4 pb-3 pt-3 text-white">
        ...
        <div className="relative flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-center gap-3">
            <div className="relative shrink-0">
              <span className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-white p-0.5 shadow-lg ring-2 ring-[#18B368]/50">
                <Image
                  src="/kebu-mascot.jpg"
                  alt=""
                  width={48}
              ...
              <span className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-2 border-[#0B1B16] bg-[#22C55E]" />
            </div>
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <select
              ...
              className="max-w-[96px] rounded-full border border-white/15 bg-white/10 px-2 py-1 text-[11px] font-bold text-white outline-none"
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-3 py-3">{children}</div>
      {chips ? <div className="border-t border-slate-200/80 bg-white/60 px-3 py-2">{chips}</div> : null}
      <div className="border-t border-slate-200/80 bg-white p-3">{footer}</div>
    </div>
  );
}
