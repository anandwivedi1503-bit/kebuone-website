...
export default function OpsMoneyStrip() {
  ...
  return (
    <div className="mb-6 overflow-hidden rounded-2xl border border-emerald-100/80 bg-white/90 p-4 shadow-[0_14px_40px_rgba(15,23,42,0.06)] sm:mb-8 sm:p-5">
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-3 xl:grid-cols-6">
        {cards.map((card) => (
          <div
            key={card.label}
            className="rounded-xl border border-slate-100 bg-gradient-to-br from-white to-emerald-50/40 px-3 py-2.5 transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-[0_10px_24px_rgba(16,185,129,0.16)]"
          >
            <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">{card.label}</p>
            <p className="mt-1 text-base font-black tabular-nums text-slate-950 sm:text-lg">{card.value}</p>
